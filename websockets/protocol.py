...
class WebSocketCommonProtocol(asyncio.StreamReaderProtocol):
    ...
    * the request and response HTTP headers are available in the
    ...
    def read_data_frame(self, max_size):
        ...
        while True:
            ...
            if frame.opcode == OP_CLOSE:
                ...
                self.debug("%s - received ping, sending pong: %s",
                             self.side, ping_hex)
                ...
                if frame.data in self.pings:
                    ...
                    self.debug("%s - received solicited pong: %s",
                                 self.side, pong_hex)
                    ping_ids = ping_ids[:-1]
                    if ping_ids:
                        pings_hex = ', '.join(
                    ...
                    pong_hex = (
                        binascii.hexlify(frame.data).decode() or '[empty]')
                    self.debug("%s - received unsolicited pong: %s",
                                 self.side, pong_hex)
    ...
    def read_frame(self, max_size):
        ...
        self.debug("%s < %s", self.side, frame)
        return frame
    ...
    def connection_lost(self, exc):
        ...
        self.debug("%s x code = %d, reason = %s", self.side,
